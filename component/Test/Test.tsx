...
export default function Test() {
  ...
  const computeResults = () => {
    ...
    questions.forEach((q, idx) => {
      const correctAnswerRaw =
        q.correct_answer ?? q.correctAnswer ?? q.correct_answer_label;

      const correctAnswer =
        correctAnswerRaw != null ? String(correctAnswerRaw) : "";
      const userChoice = answers[idx];
      if (
        userChoice != null &&
        correctAnswer !== "" &&
        String(userChoice).toUpperCase() === correctAnswer.toUpperCase()
      ) {
        correct += 1;
      }
    });
  ...
  const renderForm = () => (
    <div className="test-form-card">
      ...
      <p className="note">
        The test is generated by AI and displayed directly (not saved).
      </p>
    </div>
  );
      ...
      <div className="summary-list">
        {questions.map((q, idx) => {
          ...
          const correctAnswer =
            q.correct_answer ??
            q.correctAnswer ??
            q.correct_answer_label ??
            q.correct_answer;
          const isCorrect =
            userChoice &&
            correctAnswer &&
            userChoice.toString().toUpperCase() ===
